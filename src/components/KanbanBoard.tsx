  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div 
        ...
        className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 p-4 rounded-lg bg-white border border-slate-200 shadow-xs"
      >
        {/* Search */}
        <div className="relative w-full lg:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-md text-slate-800 placeholder-slate-400 focus:bg-white focus:border-slate-400 focus:outline-none transition shadow-2xs"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            <select
              ...
              className="text-xs bg-slate-50 border border-slate-200 rounded-md px-2.5 py-1.5 text-slate-700 focus:border-slate-400 focus:outline-none cursor-pointer transition hover:bg-slate-100/70"
            >
          ...
          <div className="flex items-center gap-2 ml-auto">
            {onOpenConflictModal && (
              <motion.button
                whileHover={{ scale: 1.025, backgroundColor: '#f8fafc' }}
                whileTap={{ scale: 0.97 }}
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-md bg-white border border-slate-200 text-slate-700 text-xs font-semibold shadow-2xs transition cursor-pointer"
              >
            ...
            {onOpenCopilot && (
              <motion.button
                whileHover={{ scale: 1.03, boxShadow: '0 2px 8px -1px rgba(245, 158, 11, 0.2)' }}
                whileTap={{ scale: 0.97 }}
                onClick={onOpenCopilot}
                title={language === 'es' ? "Asistente Jurídico de Cláusulas y Análisis" : "Legal AI Copilot & Clause Drafter"}
                className="flex items-center gap-1.5 px-3 py-2 rounded-md bg-amber-50 hover:bg-amber-100/80 border border-amber-200 text-amber-900 text-xs font-semibold shadow-2xs transition cursor-pointer relative overflow-hidden"
              >
                <motion.div
            ...
            {/* New Case Button */}
            <motion.button
              whileHover={{ scale: 1.025, backgroundColor: '#1e293b' }}
              whileTap={{ scale: 0.97 }}
              onClick={() => setShowNewCaseModal(true)}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-md bg-slate-900 text-white text-xs font-semibold tracking-wide shadow-xs transition cursor-pointer shrink-0"
            >
        ...
        {KANBAN_STAGES.map((stage, stageIndex) => {
          ...
          return (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.38, delay: stageIndex * 0.07, ease: "easeOut" }}
              className="flex flex-col rounded-lg kanban-col shadow-xs min-w-[280px] max-h-[calc(100vh-280px)] border border-slate-200/90"
            >
              {/* Column Header */}
              <div className="p-3.5 border-b border-slate-200 bg-white/70 rounded-t-lg sticky top-0 z-10 backdrop-blur-xs">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <h3 className="serif text-sm font-semibold text-slate-900 tracking-tight truncate">
                    {localizeStage(stage.id)}
                  </h3>
                  <motion.span 
                    key={stageCases.length}
                    initial={{ scale: 0.8 }}
                    animate={{ scale: 1 }}
                    className="mono text-xs px-2 py-0.5 rounded-full bg-slate-200 text-slate-700 font-semibold shadow-2xs"
                  >
                    {stageCases.length}
                  </motion.span>
                </div>
                ...
                <AnimatePresence mode="popLayout">
                  {stageCases.length === 0 ? (
                    <motion.div 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="py-8 px-4 text-center border border-dashed border-slate-300 rounded-lg text-slate-400 text-xs"
                    >
                      {t('kanban.emptyStage')}
                    </motion.div>
                  ) : (
                    stageCases.map((caseItem, caseIndex) => {
                      ...
                      const isUrgent = caseItem.priority === 'Urgente / Perentorio';
                      ...
                      return (
                        <motion.div
                          ...
                          transition={{ 
                            duration: 0.3, 
                            delay: caseIndex * 0.04,
                            ease: "easeOut"
                          }}
                          whileHover={{ 
                            y: -4, 
                            boxShadow: "0 10px 24px -6px rgba(15, 23, 42, 0.1), 0 4px 6px -2px rgba(15, 23, 42, 0.04)",
                            borderColor: "#94a3b8"
                          }}
                          whileTap={{ scale: 0.985 }}
                          onClick={() => onSelectCase(caseItem)}
                          className="group relative rounded-lg p-3.5 card transition-colors cursor-pointer text-left"
                        >
                          {/* Top: Code & Priority */}
                          <div className="flex items-center justify-between gap-2 mb-2">
                            ...
                            <span className={`text-[10px] px-2 py-0.5 rounded border flex items-center gap-1 ${getPriorityBadgeClass(caseItem.priority)}`}>
                              {isUrgent && (
                                <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-ping inline-block" />
                              )}
                              <span>{localizePriority(caseItem.priority).split('/')[0]}</span>
                            </span>
                          </div>
        ...
        {showNewCaseModal && (
          ...
          >
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-lg p-6 shadow-2xl overflow-y-auto max-h-[90vh]"
            >
              <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-200">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-slate-700 text-lg font-mono p-1 cursor-pointer transition hover:scale-110"
                >
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handleCreateCaseSubmit} className="space-y-4 text-xs">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-slate-900 mono focus:bg-white focus:border-slate-400 focus:outline-none transition"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-slate-900 focus:bg-white focus:border-slate-400 focus:outline-none transition"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-slate-900 focus:bg-white focus:border-slate-400 focus:outline-none cursor-pointer transition"
                    >
                      <option value="Litigación & Arbitraje">{localizeJurisdiction('Litigación & Arbitraje')}</option>
                      <option value="Mercantil & Societario">{localizeJurisdiction('Mercantil & Societario')}</option>
                      <option value="M&A / Fusiones y Adquisiciones">{localizeJurisdiction('M&A / Fusiones y Adquisiciones')}</option>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-md text-slate-900 placeholder-slate-400 focus:bg-white focus:border-slate-400 focus:outline-none transition"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.97 }}
                    onClick={() => setShowNewCaseModal(false)}
                    className="px-4 py-2 rounded-md border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 font-medium cursor-pointer transition"
                  >
                    {t('action.cancel')}
                  </motion.button>
                  <motion.button
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.97 }}
                    className="px-5 py-2 rounded-md bg-slate-900 hover:bg-slate-800 text-white font-medium shadow-xs cursor-pointer transition"
                  >
                    {t('case.modal.submit')}
                  </motion.button>
                </div>
